...
function drawGraph()
{
	...
	for(var i=0;i<size;i++)
	{
		...
		var cir=raph.circle(windowx+Level3.x[i],windowy+Level3.y[i],10);
		...
		if(i==Level3.source)
		{
			raph.text(windowx+Level3.x[i],windowy+Level3.y[i]-20,"S",raph.getFont("Courier"));
			cir.attr({fill:"45-brown-rgb(182, 11, 248)"});
			//document.write(Level3.source);
		}
		if(i==Level3.destination)
		{
			raph.text(windowx+Level3.x[i],windowy+Level3.y[i]-20,"D",raph.getFont("Courier"));
			cir.attr({fill:"45-brown-rgb(182, 11, 248)"});
...
function drawEdges()
{
	...
	for(var i=0;i<Level3.no_of_vertices;i++)
	{
		for(var j=0;j<Level3.no_of_edges[i];j++)
		{
			...
			var startx=windowx+Level3.x[i];
			var starty=windowy+Level3.y[i];
			var endx=windowx+Level3.x[ll];
			var endy=windowy+Level3.y[ll];
...
function drawBestPath(bestStack)
{
	for(var i=0;i<bestStack.length-1;i++)
	{
		var l=bestStack[i];
		var m=bestStack[i+1];
			var startx=windowx+Level3.x[l];
			var starty=windowy+Level3.y[l];
			var endx=windowx+Level3.x[m];
			var endy=windowy+Level3.y[m];
...
function getClickPosition(e) 
{
	...
	for(var i=0;i<Level3.no_of_edges[presentvertex];i++)
	{
    	...
    	{
    		var startx=windowx+Level3.x[presentvertex];
			var starty=windowy+Level3.y[presentvertex];
